...
def train_model(base_lr, loss, data_num, global_step):
    """
    train model
...
def train(net_factory, prefix, end_epoch, base_dir,
          display=200, base_lr=0.01, restore=False):
    """
    :param net_factory:  PNet/RNet/ONet
    :param prefix: ckpt path
    :param end_epoch: max epoch to train
    :param base_dir: dataset path, eg. '../../DATA/imglists/PNet'
    :param display:
    :param base_lr:
    :return:
    """
    net = prefix.split('/')[-1]
    # label file
    label_file = os.path.abspath(os.path.join(base_dir, 'train_%s_landmark.txt' % net))
    ...
    ops = net_factory(input_image, label, bbox_target, landmark_target, training=True)
    cls_loss_op, bbox_loss_op, landmark_loss_op, L2_loss_op, accuracy_op = ops
    # train,update learning rate(3 loss)
    total_loss_op = radio_cls_loss * cls_loss_op + radio_bbox_loss * bbox_loss_op + radio_landmark_loss * landmark_loss_op + L2_loss_op
    global_step = tf.Variable(0, trainable=False)
    train_op, lr_op = train_model(base_lr, total_loss_op, num, global_step)
    # init
    ...
    sess.run(init)
    if restore:
        ckpt = tf.train.get_checkpoint_state(prefix)
            ...
            if step % display == 0:
                ...
                print(
                    "Step: %d/%d, accuracy: %3f, cls loss: %4f, bbox loss: %4f,Landmark loss :%4f,L2 loss: %4f, Total Loss: %4f ,lr:%f " % (
                        step, MAX_STEP, acc, cls_loss, bbox_loss, landmark_loss, L2_loss,
                        total_loss, lr))

            # save every two epochs
            if step % 1000 == 0:
                path_prefix = saver.save(sess, os.path.join(prefix, "pnet.ckpt"), global_step=step)
