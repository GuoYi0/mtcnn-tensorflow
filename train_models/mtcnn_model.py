...
def cls_ohem(cls_prob, label):
    """
    对于人脸检测。 只有正例与负例参与训练，part和landmark不予训练
    iou>0.65, 正例， 标签1
    ...
    label_filter_invalid = tf.where(tf.less(label, 0), zeros, label)  # [bs]
    num_cls_prob = tf.size(cls_prob)
    cls_prob_reshape = tf.reshape(cls_prob, [num_cls_prob, -1])
    label_int = tf.cast(label_filter_invalid, tf.int32)  # [bs]
    # get the number of rows of class_prob
    num_row = tf.to_int32(cls_prob.get_shape()[0])
    # row = [0,2,4.....]
    row = tf.range(num_row) * 2  # [0, 2, 4, ..., 2*bs-2]
    indices_ = row + label_int
    label_prob = tf.squeeze(tf.gather(cls_prob_reshape, indices_))  # [bs]
...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        net = slim.conv2d(inputs, 10, 3, stride=1, scope='conv1')  # 10x10x10
        _activation_summary(net)
        net = slim.max_pool2d(net, kernel_size=[2, 2], stride=2, scope='pool1', padding='SAME', data_format=DATA_FORMAT)  # 5x5x10
        _activation_summary(net)
        net = slim.conv2d(net, num_outputs=16, kernel_size=[3, 3], stride=1, scope='conv2')  # 3x3x16
        _activation_summary(net)
        net = slim.conv2d(net, num_outputs=32, kernel_size=[3, 3], stride=1, scope='conv3')  # 1x1x32
        _activation_summary(net)
        conv4_1 = slim.conv2d(net, num_outputs=2, kernel_size=[1, 1], stride=1, scope='conv4_1',  # cls
                              activation_fn=tf.nn.softmax)
        _activation_summary(conv4_1)

        bbox_pred = slim.conv2d(net, num_outputs=4, kernel_size=[1, 1], stride=1, scope='conv4_2', activation_fn=None)  # bbox
        _activation_summary(bbox_pred)
        # batch*H*W*10
        landmark_pred = slim.conv2d(net, num_outputs=10, kernel_size=[1, 1], stride=1, scope='conv4_3',
                                    activation_fn=None)  # landmarks
        ...
        if training:
            ...
            cls_prob = tf.squeeze(conv4_1, axis, name='cls_prob')
            cls_loss = cls_ohem(cls_prob, label)
            # batch
            # cal bounding box error, squared sum error
            bbox_pred = tf.squeeze(bbox_pred, axis, name='bbox_pred')
            bbox_loss = bbox_ohem(bbox_pred, bbox_target, label)
            # batch*10
            landmark_pred = tf.squeeze(landmark_pred, axis, name="landmark_pred")
