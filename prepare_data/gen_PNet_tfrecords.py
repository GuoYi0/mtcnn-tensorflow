...
def get_dataset(dir, net='PNet'):
    ...
    for line in lines:
        ...
        data_example['label'] = int(info[1])  # pos 1， neg 0， part -1， ldms -2
        bbox['xmin'] = 0
        bbox['ymin'] = 0
        bbox['xmax'] = 0
        bbox['ymax'] = 0
        ...
        bbox['xrightmouth'] = 0
        bbox['yrightmouth'] = 0
        if len(info) == 6:  # 只有框，没有landmark
            bbox['xmin'] = float(info[2])
            bbox['ymin'] = float(info[3])
            bbox['xmax'] = float(info[4])
            bbox['ymax'] = float(info[5])
        if len(info) == 12:  # 只有landmark，没有框
...
if __name__ == '__main__':
    ...
    run(dir_, net, output_directory, shuffling=True)
