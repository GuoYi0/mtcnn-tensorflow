...
with open(os.path.join(dir_path, "%s" % net, "train_%s_landmark.txt" % net), "w") as f:
    ...
    neg_keep = list(range(len(neg)))
    shuffle(neg_keep)
    # shuffle the order of the initial data
    # if negative examples are more than 750k then only choose 750k
    # neg_keep = shuffle([i for i in range])
    if len(neg) > base_num * 3:
        # neg_keep = npr.choice(len(neg), size=base_num * 3, replace=True)
        neg_keep = neg_keep[:base_num * 3]
    pos_keep = list(range(len(pos)))
    shuffle(pos_keep)
    pos_keep = pos_keep[:base_num]

    part_keep = list(range(len(part)))
    shuffle(part_keep)

    part_keep = part_keep[:base_num]
