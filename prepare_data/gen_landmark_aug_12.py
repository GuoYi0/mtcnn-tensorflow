# coding: utf-8
"""
根据lfw数据集，制作landmark训练集。只有landmark。没有框
"""
...
def GenerateData(ftxt, data_path, net, argument=False):
    ...
    for (imgPath, bbox, landmarkGt) in data:
        ...
        # 可视化
        # cv2.rectangle(img, (gt_box[0], gt_box[1]), (gt_box[2], gt_box[3]), (255, 0,0), 3)
        # for lan in landmarkGt:
        #     cv2.circle(img, (int(lan[0]), int(lan[1])), 2, (0,0,255))
        # cv2.imshow("hah", img)
        # cv2.waitKey(0)
        # continue
        #
        ...
        if argument:
            ...
            for i in range(10):
                ...
                delta_x = npr.randint(int(-gt_w * 0.2), gt_w * 0.2)
                delta_y = npr.randint(int(-gt_h * 0.2), gt_h * 0.2)
